...
if (!class_exists('ThemeToolkit')) {
	class ThemeToolkit{
		...
		function add_menu() {
			...
			add_theme_page('Configure ' . $this->infos['theme_name'], $this->infos['theme_name'] . ' Options', 'edit_themes', basename($this->file), array(&$this,'admin_menu'));
		...
		function admin_menu () {
			...
			echo '<h2>Configure ',$this->infos['theme_name'],' Theme</h2>';
			...
			foreach ($this->infos['menu_options'] as $key=>$val) {
				...
				if (@$items) {
					...
					case 'textarea':
						...
						echo '><label for="',$key,'">',$matches[1],'</label></th>',"\n",'<td>';
					...
					echo '><label for="',$key,'">',$matches[1],'</label></th>',"\n",'<td>
						<input type="text" name="',$key,'" id="',$key,'" value="',$this->option[$key],'" />';
				}

				if ($matches[5]) echo '<br />'. $matches[5];
				echo "</td></tr>\n";
			}
			echo '</tbody>
			</table>
